...
try:
    ...
    class VaultSecretEngineModule(VaultModule):
        ...
        DURATION_PARAMS: List[str] = ["default_lease_ttl", "max_lease_ttl"]
        NON_MOUNT_CONFIG_PARAMS: List[str] = [
            "engine_mount_point",
            "state",
        ...
        def get_defined_mount_config_params(self) -> dict:
            ...
            delete_keys: List[str] = [
                key for key in filtered_params.keys() if key not in self.ARGSPEC
            ]
            ...
            delete_keys: List[str] = [
                key for key in filtered_params.keys() if key in self.NON_MOUNT_CONFIG_PARAMS
            ]
            ...
            delete_keys: List[str] = [
                key for key in filtered_params.keys() if self.params[key] is None
            ]
        ...
        def get_mount_backend_type(self) -> Optional[str]:
            ...
            try:
                mounted_engines: dict = self.client.sys.list_mounted_secrets_engines().get(
                    "data",
                    {},
                )
            except Forbidden:
        ...
        def compare_mount_config(self, previous_config: dict, desired_config: dict) -> dict:
            ...
            for key, value in desired_config.items():
                if key not in previous_config:
                    ...
                    elif key == "default_lease_ttl" or key == "max_lease_ttl":
                        if value != self.DEFAULT_TTL:
                            differences[key] = value
                    elif (
                        key == "audit_non_hmac_request_keys"
                        or key == "audit_non_hmac_response_keys"
                        or key == "passthrough_request_headers"
                    ):
                        if value != []:
                            differences[key] = value
