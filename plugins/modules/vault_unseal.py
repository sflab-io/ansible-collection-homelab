...
from ansible_collections.community.hashi_vault.plugins.module_utils._connection_options import (
    ...
    class VaultUnsealModule(HashiVaultModule):
        ...
        def get_defined_non_connection_params(self) -> dict:
            ...
            delete_keys: list[str] = [
                key for key in self.params.keys() if key in HashiVaultConnectionOptions.ARGSPEC
            ]
            ...
            delete_keys: list[str] = [
                key for key in filtered_params.keys() if filtered_params[key] is None
            ]
...
def run_module():
    ...
    if not module.client.sys.is_initialized():
        module.handle_error(
            VaultModuleError(message="Cannot unseal a Vault that has not been initialized."),
        )
    ...
    if module.client.sys.is_sealed():
        ...
        if not module.check_mode:
            ...
            for key in module.params["unseal_keys"]:
                unseal_response = module.client.sys.submit_unseal_key(key=key, reset=reset)
